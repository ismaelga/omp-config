...
Answering outside the options stays open to the user at all times, so a short option set never boxes them in. Never write your own escape hatch: `Other (type your own)`, `Chat about this` and `Next →` are labels the runtime owns, and reusing one collides with its controls. `ask` also runs alone in its tool batch, so a round is a call of its own — and if `ask.timeout` is set, an unanswered question auto-selects `recommended`, which is the other reason every question carries a real one.
