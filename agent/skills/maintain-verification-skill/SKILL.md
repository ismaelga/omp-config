...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.omp/skills/verify-*/`). Several candidates → use the `ask` tool to pick one; none → stop and point at `/skill:create-verification-skill` instead of inventing a target.
...
2. **Source wave.** One `scout` subagent per feature file, launched concurrently as one `task` batch (one `tasks[]` entry each: `{name, agent: "scout", task}`). Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. `scout` is read-only; children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe. Results auto-deliver in the batch; do not poll.
...
6. **Ship or stop.** For changed: one PR of proven corrections, re-read every changed file first. Open it with the `github` device (`pr_create`), and watch CI with `run_watch`, which fast-fails on the first failing job and saves the logs to an artifact. For clean or blocked: no PR, report the outcome and the coverage honestly.
