...
Run the baseline against your real eval (see `eval-driven-development`). Record:
...
- **`eval-driven-development`** → The eval tells you whether the baseline is enough
...
- **`vibe-coding-guardrails`** → Slop is often skipping baseline and over-engineering
