...
Scope: one glyph swapping for another inside a control that stays put. The `scale: 0.25`
start below is deliberate and applies only to that swap -- a glyph has no layout presence
to preserve, so it can grow from almost nothing. It is not a licence to break `animate`'s
rule for elements entering the page, which start at `scale(0.9-0.97)`.
