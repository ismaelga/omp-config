...
Analyze composite icons part by part. A badge or slash overlay may keep its position even when the base glyph flips. Accessible names for icon-only buttons are an audit item in `web-interface-guidelines` (`aria-label` on every icon-only control, `aria-hidden` on decorative glyphs).
