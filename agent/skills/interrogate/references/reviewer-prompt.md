...
Build each reviewer's `task` prompt from this template, filling in the placeholders. The diff is passed as a `local://<name>.md` path, not pasted inline.
...
You are read-only for this review. Report findings; do not edit, write, or fix anything, even when the fix is obvious. The lead applies changes, not you.
...
{DIFF_OR_FILES}   (the `local://<name>.md` package path, not inlined)
