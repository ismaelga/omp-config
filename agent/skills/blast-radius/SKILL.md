...
Companion to a code walkthrough (what the code does — a `task`/`scout` dispatch with an explainer brief) and the git/PR archaeology below (why it is shaped that way). Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the history behind it: `git blame -L <start>,<end> <file>`, `git log --follow -p -- <file>`, and the PR body for any merge commit (`(#1234)` in the subject).
