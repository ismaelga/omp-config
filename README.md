...
| `sentinel` | `opencode-go/glm-5.3:high` | security review is long-context recall over a diff *plus its callers* — 1M context. Same weights as the `zai` pin it replaces, on the provider with room: opencode-go reads 0% of its 5-hour, 11% of its weekly and 7% of its monthly allowance (2026-09-11), and it keeps one reviewer off the `ollama-cloud` queue that seven of the twelve roles now share — `task`, `scout`, `smol`, `vision`, `advisor`, `tiny`, `commit`. That second reason is the durable one: zai's week has since reset, so headroom no longer separates the two providers |
...
| `advisor` | `ollama-cloud/glm-5.3-flash:high` | live, not inert — see [what is turned off](#what-is-turned-off-and-why). Advisor cost is uncached *input*, so prefill and TTFT decide it, not reasoning rank. Same weights as the `zai` pin it replaces and faster on both axes: 3671 ms TTFT / 68.5 tok/s (n=140) against zai's 5143 ms across the 116 of its 148 samples that carry a TTFT, at 32.1 tok/s — and zai's row has been frozen since 2026-09-06, since nothing routes there now |

Model specs are `provider/model[:thinking]` where thinking ∈ `minimal|low|medium|high|xhigh|max`.

Every `model_perf` figure above is a snapshot of a **decaying rolling window**, read 2026-09-11, not a running total: `samples` is a decayed weight (fractional — zai's row reads 148.03), `ttft_samples` is a separate and often smaller denominator, and a row that stops receiving calls freezes at its last average. The `ollama-cloud/glm-5.3-flash` row moved 2252 → 3671 ms between 2026-09-10 and 2026-09-11, and the `gpt-oss:120b` row moved 207.7 → 201.9 tok/s inside a single session. Directions have survived every read; the constants have not. Re-read with `sqlite3 ~/.omp/agent/agent.db` rather than quoting this table — `skills/reviewing-model-pins/commands.md` has the query.
